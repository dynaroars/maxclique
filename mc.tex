%put some more explanations on how ants move according to "surrounding information" - search for "amcs"
%todo urgent: dist result really is not comparable to those from sequential - some are only 65% (san)

%give a clear description that have to parallel correctly, cannot just parallel anywhere. Have to make sure it's safe and sound.  in this paper we parallel the ants step since it is natural and as it turns out, it is also safe. In parts where we cannot parallel to shorten the time, we parallel to get better solution quality (e.g., faster convergences)
...
In~\cite{BR}, Bui et al. used a variant of ant based algorithms called \textit{Ant System} to solve the \textit{Max-Clique} problem. Their conclusion offered brief intuitions on an implementation of the algorithm in distributed computing.  In this paper, we describe the process of transforming their sequential algorithm into two popular parallel environments, namely the shared and distributed memory models. We hope these ideas serve as proofs of concept that parallel programming is ideal for algorithms inspired by social insects. %todo check et al
...
keller4      	&171/9435 		 &3.40/1.01     &2.52/1.32     &1.99/1.67   &1.68/1.98\\  %3.32            
...
\hline                                                                                   
sanr200\_0.7 	&200/13868 	     &4.09/1.03 &2.84/1.49 &2.40/1.76 &2.13/1.98\\  %4.22
...
%We developed the program on a multiprocessor computer provided by the HP Test Drive~\cite{HPTestDrive} program. The Linux based system is equipped with sixteen Intel Itanium $1.5$ GHz processors and $8$ GB memory.
