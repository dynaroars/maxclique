...
%It has been shown that even finding good approximation to instance of Max-Clique in polynomial time are impractical as the margin of error can grow quite high

%The Max-Clique problem belongs to the $\cal NP$-hard class and hence, exact polynomial time solvers for them are not expected to exist. Likewise, existing approximation algorithms for the problem in polynomial time are impractical as the margin of error can grow quite high. Therefore, heuristics such as ant based algorithms are needed to find good solutions in a resonable amount of time.
...
The common programming paradigm for shared memory architecture is the multithread \textit{fork-join} model. A single \textit{Master} thread is created when the program starts, executes sequentially, and expands into multiple \textit{Slave} threads when parallelization is requested. These threads work concurrently through the parallel region then merge back to the single master thread when done with the parallel section.

To use the \textit{fork-join} model, parallel regions from the sequential code need to be specified. Sections in ASMC that are efficient and safe for parallelism, i.e., reducing the execution time while not degrading the solution quality, make good candidates. The \textit{stage} and \textit{cycle} loops do not fit these criteria since each stage requires configurations from previous stage such as locations of ants and pheromone info and per time cycle, an ant can only perform an action (movement).  We then designate the \textit{ant} section as the main parallel regions as concurrent processing on this area does not affect the quality of the results.  Moreover, this part consumes more than half of the total processing time in AMSC~\footnote{The GNU profile utility \textit{grof} was used to identy time consuming operations in our program.}. AS algorithms often use a much larger ant population than traditional ACO; in ASMC, the size of the ant colony is approximately six times the number of vertices from the input graph.
...
%see how the entire paper look and perhaps expand this part !
...
\item The server {\bf ends} by letting each client know that the run has ended.  %Synchonized starting and ending makes it simpler to invoke the program from a script.
...
\newblock {Journal of the Operational Research Society}, Volume 50, February 1999, pp. 167--176.
