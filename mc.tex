...
+%todo: incoporate something about why AS is more ammenable to parallel computing than traditional ACO.  These Ant based algorithms are very appealing to parallel computing because.  Morever Ant System is even more ammenable since the ants doesn't use local information and ....
...
%todo: words to use paradigm, synchronous (happens concurrently)
...
%In this section we summarize our sequential Ant System algorithm for Max-Clique (ASMC) then discuss the parallel shared and distributed versions of ASMC.
...
Techniques such periodical pheromone evaporations and configuration perturbations (shuffling the ants) are used in the algorithm to help escape local optima.  Moreover, adaptive behaviors are given to the ants in ASMC to encourage a gradual transformation from exploration in the beginning to exploitation near the end. In beginning, young ants are more active and rely less on pheromone and more on the structure of the graph.  As the algorithm progresses, ants get older and are less likely to move and they make more use of pheromone in determining their movement. %todo: shorten

Complete descriptions of each component in ASMC can be found in~\cite{BR}.
...
\Begin{
  \medskip
  Use a \myfont{greedy} method to get a \myfont{clique} $q$ in $G$\;
  ...
  \For{$s = 1$ {\bf{to}} \myfont{nStages}}{
    \For{$c = 1$ {\bf{to}} \myfont{nCycles}}{
      \For{$a = 1$ {\bf{to}} \myfont{nAnts}}{
        ...
        ant $a$ moves to new destination and deposits pheromone on the traversed edge\;
      }
    }
  ...
  \begin{algorithm}
    ...
    \Begin{
      \For{$c = 1$ {\bf{to}} \myfont{nCycles}}{
        ...
        \For{$a = 1$ {\bf{to}} \myfont{nAnts}}{
          ant $a$ decides its next destination, vertex v$_a$, based on surrounding information\;
        }
        ...
        \For{$a = 1$ {\bf{to}} \myfont{nAnts}}{
          ant $a$ moves to new destination, vertex v$_a$, and deposits \myfont{pheromone} on the way\;
        }
        \medskip
      }
...
\subsection{The Distributed Implementation (ASMC$_{dist}$)}\label{ASMC_dist} 
